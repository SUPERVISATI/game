...
// ============================================================
// AUDIO MANAGER - COM VOLUMES AJUSTADOS CORRETAMENTE
// ============================================================
...
class AudioManager {
    constructor() {
        ...
        // ============================================================
        // VOLUMES RELATIVOS - AJUSTADOS PARA HIERARQUIA CORRETA
        // ============================================================
        this.VOLUME_MUSIC = 0.20;      // Música de fundo
        this.VOLUME_COIN = 0.90;       // Coleta de moeda - MAIS ALTO
        this.VOLUME_HIT = 0.20;        // Acerto de alvo
        this.VOLUME_SHOOT = 0.05;      // Disparo - MAIS BAIXO
        this.VOLUME_BOSS = 0.50;       // Chefão derrotado
        this.VOLUME_BOSS_HIT = 0.90;   // Dano no chefão
        this.VOLUME_BOSS_SHOOT = 0.90; // Ataque do chefão
    ...
    loadNewMusic(level, filePath) {
        ...
        this.loadAudio(filePath, (audio) => {
            ...
            if (audio) {
                ...
                this.currentMusic.loop = true; // Música em loop
    ...
    playMusic() {
        if (this.currentMusic && !this.isMusicPlaying && this.musicLoaded) {
            try {
                ...
                let volume = 0;
                const targetVolume = this.VOLUME_MUSIC;
                this.fadeInterval = setInterval(() => {
                    volume += 0.05;
                    if (this.currentMusic) {
    ...
    playBossAttackSound(attackType) {
        ...
        try {
            ...
            audio.volume = this.VOLUME_BOSS_SHOOT;
            audio.play().catch(e => {});
        } catch (e) {}
    }
    ...
    playBossHitSound() {
        ...
        try {
            const audio = new Audio(filePath);
            audio.volume = this.VOLUME_BOSS_HIT;
            audio.play().catch(e => {});
        } catch (e) {}
    }
    ...
    playCoinSound() {
        ...
        try {
            const audio = new Audio(filePath);
            audio.volume = this.VOLUME_COIN; // Volume mais alto!
            audio.play().catch(e => {});
        } catch (e) {}
    }
    ...
    playSoundEffect(type) {
        let filePath = null;
        let volume = this.VOLUME_SHOOT;
        
        if (type === 'shoot') {
            filePath = this.soundEffects.shoot;
            volume = this.VOLUME_SHOOT;
        } else if (type === 'hitTarget') {
            filePath = this.soundEffects.hitTarget;
            volume = this.VOLUME_HIT;
        } else if (type === 'bossDefeat') {
            filePath = this.soundEffects.bossDefeat;
            volume = this.VOLUME_BOSS;
        } else if (type === 'bossShoot') {
            filePath = this.soundEffects.bossShoot;
            volume = this.VOLUME_BOSS_SHOOT;
        }
    ...
    setVolume(volume) {
        ...
        if (this.currentMusic) {
            this.currentMusic.volume = this.VOLUME_MUSIC;
        }
    }
...
// ============================================================
// SISTEMA DE BALÃO - APENAS FORMAS GEOMÉTRICAS SEM ÍCONES OU TEXTOS
// ============================================================

// Configurações dos 7 balões - mantido apenas para as cores e formas
const BALLOON_TYPES = {
    double_shot: {
        id: 'double_shot',
        name: '',
...
let balloonSpawnQueue = [];
let spawnedBalloons = new Set();
let balloonSpawnCooldown = 0;
let totalBalloonsSpawned = 0;
const MIN_KILLS_BETWEEN_BALLOONS = 5;
const MAX_KILLS_BETWEEN_BALLOONS = 20;
...
function spawnBalloon() {
    ...
    balloonSpawnCooldown = Math.floor(Math.random() * 3) + 1;
    return true;
}

// Classe do Balão - APENAS A FORMA GEOMÉTRICA, SEM ÍCONES OU TEXTOS
class Balloon {
    ...
    draw() {
        ...
        // DESENHA APENAS A FORMA - SEM ÍCONES OU TEXTOS
...
function activateBalloonEffect(typeId) {
    ...
    // REMOVIDO: indicador visual e mensagens
}
...
function clearBalloonEffect() {
    ...
    // REMOVIDO: remoção de indicador visual
}
...
const LEVEL_TARGETS = {
    1: 220, 2: 205, 3: 190, 4: 185, 5: 180,
    6: 170, 7: 161, 8: 154, 9: 144, 10: 133,
    11: 120, 12: 109, 13: 104, 14: 100, 15: 94,
    16: 90, 17: 84, 18: 79, 19: 75, 20: 70
};
...
console.log('🎈 BALÕES: APENAS FORMAS GEOMÉTRICAS');
console.log('  ✅ Sem ícones (⚡, 🔱, 💥, 👹, etc.)');
console.log('  ✅ Sem textos flutuantes');
console.log('  ✅ Sem indicadores de efeito ativo no topo');
console.log('  ✅ Sem mensagens de status no centro');
console.log('  ✅ Apenas as formas geométricas coloridas');
